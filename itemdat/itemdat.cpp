...
#include "../utils/binarystreamreader.hpp"

bool Itemdat::LoadFromFile(const std::string& file_name) {
...
void Itemdat::LoadFromReader(Utils::BinaryStreamReader& reader) {
...
std::string Itemdat::DecryptString(ItemID item_id, const std::vector<uint8_t>& buffer) {
...
ItemInfo Itemdat::GetItemInfo(uint32_t item_id) {
    auto it = m_item_info_map.find(item_id);

    return it == m_item_info_map.end() ? ItemInfo{} : std::move(it->second);
}

ItemInfo Itemdat::parse_next_item_info(Utils::BinaryStreamReader& reader) {
    ItemInfo ret{};
